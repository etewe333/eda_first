...
int main(int argc, char** argv) {
    ...
    test_alu(1, "ADD", 0x0, 5, 3, 8, 0);

    // Test 2: SUB basic (10 - 4 = 6)
    test_alu(2, "SUB", 0x1, 10, 4, 6, 0);

    // Test 3: AND (0xFF0F & 0xF0FF = 0xF00F)
    test_alu(3, "AND", 0x2, 0xFF0F, 0xF0FF, 0xF00F, 0);

    // Test 4: OR (0x0F0F | 0xF0F0 = 0xFFFF)
    test_alu(4, "OR", 0x3, 0x0F0F, 0xF0F0, 0xFFFF, 0);

    // Test 5: XOR (0xAAAA ^ 0x5555 = 0xFFFF)
    test_alu(5, "XOR", 0x4, 0xAAAA, 0x5555, 0xFFFF, 0);

    // Test 6: SLT (5 < 10 -> 1)
    test_alu(6, "SLT", 0x5, 5, 10, 1, 0);

    // Test 7: SLT (10 < 5 -> 0, result=0, so zero=1)
    test_alu(7, "SLT", 0x5, 10, 5, 0, 1);

    // === Arithmetic Wrap-around Tests (RV32I Standard) ===

    // Test 8: ADD wrap-around (0x7FFFFFFF + 1 = 0x80000000)
    test_alu(8, "ADD", 0x0, 0x7FFFFFFF, 1, 0x80000000, 0);

    // Test 9: SUB wrap-around (0x80000000 - 1 = 0x7FFFFFFF)
    test_alu(9, "SUB", 0x1, 0x80000000, 1, 0x7FFFFFFF, 0);

    // Test 10: ADD zero (-1 + 1 = 0, zero flag = 1)
    test_alu(10, "ADD", 0x0, 0xFFFFFFFF, 1, 0, 1);

    // === SLT Signed Logic Tests (Critical) ===

    // Test 11: SLT signed (+1 < -1 -> 0, result=0, so zero=1)
    test_alu(11, "SLT", 0x5, 1, 0xFFFFFFFF, 0, 1);

    // Test 12: SLT signed (-2 < -1 -> 1)
    test_alu(12, "SLT", 0x5, 0xFFFFFFFE, 0xFFFFFFFF, 1, 0);

    // Test 13: SLT signed (equal -> 0)
    test_alu(13, "SLT", 0x5, 0x80000000, 0x80000000, 0, 1);

    // === Zero Flag Tests ===

    // Test 14: AND produces zero (1 & 0 = 0)
    test_alu(14, "AND", 0x2, 1, 0, 0, 1);

    // Test 15: XOR equal values (0x80000000 ^ 0x80000000 = 0)
    test_alu(15, "XOR", 0x4, 0x80000000, 0x80000000, 0, 1);

    // Test 16: SUB equal values (10 - 10 = 0)
    test_alu(16, "SUB", 0x1, 10, 10, 0, 1);

    // === Additional Edge Cases ===

    // Test 17: AND with negative (0x80000000 & 0x80000000 = 0x80000000)
    test_alu(17, "AND", 0x2, 0x80000000, 0x80000000, 0x80000000, 0);

    // Test 18: OR with negative (0x80000000 | 0x7FFFFFFF = 0xFFFFFFFF)
    test_alu(18, "OR", 0x3, 0x80000000, 0x7FFFFFFF, 0xFFFFFFFF, 0);

    // Test 19: XOR with negative (0x55555555 ^ 0xFFFFFFFF = 0xAAAAAAAA)
    test_alu(19, "XOR", 0x4, 0x55555555, 0xFFFFFFFF, 0xAAAAAAAA, 0);

    // Test 20: SLT negative vs positive (-1 < 1 -> 1)
    test_alu(20, "SLT", 0x5, 0xFFFFFFFF, 1, 1, 0);
